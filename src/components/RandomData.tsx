...
//           phone = `+995 ${faker.phone.number("##-##-####")}`; // Fixed phone format
//           break;
//         case "Poland":
//           name = `${faker.person.firstName()} ${faker.person.lastName()}`;
//           address = `${faker.location.streetAddress()}, ${faker.location.city()}, Poland`;
//           phone = `+48 ${faker.phone.number("###-###-###")}`; // Fixed phone format
//           break;
//         case "USA":
//         default:
//           break;
    ...
    try {
      ...
      switch (region) {
        case "Georgia (Country)":
          name = `${faker.person.firstName()} ${faker.person.lastName()}`;
          address = `${faker.location.city()}, ${faker.location.country()}`;
          phone = faker.phone.number({ style: "international" }); // Fixed phone format
          break;
        case "Poland":
          name = `${faker.person.firstName()} ${faker.person.lastName()}`;
          address = `${faker.location.streetAddress()}, ${faker.location.city()}, Poland`;
      ...
      <div className="max-w-3xl mx-auto p-5">
        ...
        <div className="mb-4">
          ...
          <input
            ...
            value={sliderValue}
            onChange={handleSliderValue}
            min={0} // Changed to number
            max={10} // Changed to number
            className="w-full mb-2"
            aria-label="Error Rate Slider"
            aria-valuemin={0} // Changed to number
            aria-valuemax={10} // Changed to number
            aria-valuenow={sliderValue}
          />
          <span className="inline-block mb-2">{sliderValue}</span>
          <input
        ...
        <div>
          ...
          {loading && (
            <div
              ref={loadingRef}
              className="text-center p-4 border border-blue-500"
            >
              Loading more users...
            </div>
          )}
